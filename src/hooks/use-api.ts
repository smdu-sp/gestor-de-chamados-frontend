...
export function usePaginatedApi<T>(
  apiCall: (pagination: PaginationParams, filters?: any) => Promise<PaginatedResponse<T>>,
  ...
  const changeSorting = useCallback((sortBy: string, sortOrder: 'asc' | 'desc' = 'desc') => {
...
// Calls hooks
export function useCalls(initialPagination?: Partial<PaginationParams>, initialFilters?: CallFilters) {
...
export function useCallMutations() {
  ...
  const assignCall = useMutation(({ callId, technicianId }: { callId: number; technicianId: number }) =>
    CallsService.assignCall(callId, technicianId)
  )
  const changeStatus = useMutation(({ callId, status, resolution }: { callId: number; status: any; resolution?: string }) =>
...
export function useMyAssignedCalls(initialPagination?: Partial<PaginationParams>, initialFilters?: Omit<CallFilters, 'assignedTo'>) {
  return usePaginatedApi(
...
// Users hooks
export function useUsers(initialPagination?: Partial<PaginationParams>, initialFilters?: UserFilters) {
  return usePaginatedApi(
...
export function useDashboardCharts(filters?: any) {
  const callsByStatus = useApi(() => DashboardService.getCallsByStatus(filters), [filters])
  const callsByPriority = useApi(() => DashboardService.getCallsByPriority(filters), [filters])
  const callsByCategory = useApi(() => DashboardService.getCallsByCategory(filters), [filters])
  const technicianPerformance = useApi(() => DashboardService.getTechnicianPerformance(filters), [filters])
  const monthlyTrends = useApi(() => DashboardService.getMonthlyTrends(filters), [filters])
...
export function useRealTimeDashboard(filters?: any, refreshInterval: number = SERVICES_CONFIG.DASHBOARD_REFRESH_INTERVAL_MS) {
  ...
  }, [isAutoRefresh, refreshInterval, dashboardStats.refetch, realTimeMetrics.refetch])
...
// Notifications hooks
export function useNotifications(initialPagination?: Partial<PaginationParams>, filters?: any) {
  return usePaginatedApi(
...
export function useRecentNotifications(limit: number = 10) {
  return useApi(() => NotificationsService.getRecentNotifications(limit), [limit])
}
...
export function useNotificationMutations() {
  ...
  const deleteNotification = useMutation(NotificationsService.deleteNotification)
  const createNotification = useMutation(NotificationsService.createNotification)
...
export function useSettingsMutations() {
  ...
  const updateGeneralSettings = useMutation(SettingsService.updateGeneralSettings)
  const updateSecuritySettings = useMutation(SettingsService.updateSecuritySettings)
  const updateNotificationSettings = useMutation(SettingsService.updateNotificationSettings)
...
export function useDebouncedSearch<T>(
  searchFn: (query: string, pagination: PaginationParams, filters?: any) => Promise<PaginatedResponse<T>>,
  delay: number = 300,
  initialPagination?: Partial<PaginationParams>,
  initialFilters?: any
) {
...
export function useFileUpload() {
  ...
  const upload = useMutation(
    async ({ callId, file, description }: { callId: number; file: File; description?: string }) => {
      setIsUploading(true)
      setUploadProgress(0)
      ...
      try {
        const result = await CallsService.uploadAttachment(callId, file, description)
