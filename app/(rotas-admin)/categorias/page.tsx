...
import { ICategoria, IPaginadoCategoria } from '@/types/categoria';
import { Suspense } from 'react';
import { columns } from './_components/columns';
import ModalUpdateAndCreate from './_components/modal-update-create';

export default async function CategoriaSuspense({
	searchParams,
}: {
	...
	return (
		<Suspense fallback={<TableSkeleton />}>
			<Categorias searchParams={searchParams} />
		</Suspense>
	);
}
...
async function Categorias({
	...
	let dados: ICategoria[] = [];

	const session = await auth();
	if (session && session.access_token) {
		const response = await categoria.buscarTodasCategorias(
		...
		if (ok) {
			if (data) {
				const paginado = data as IPaginadoCategoria;
